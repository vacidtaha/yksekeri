...
export default function TytTurkcePage() {
  ...
  const topics: Topic[] = [
    {
      ...
      subTopics: [
        ...
        { id: "paragrafta-anlam", title: "Paragrafta Anlam", hasVideo: true, hasPDF: true, hasQuiz: true },
        { id: "ses-bilgisi", title: "Ses Bilgisi", hasVideo: true, hasPDF: true, hasQuiz: true },
        { id: "yazim-kurallari", title: "Yazım Kuralları (İmla)", hasVideo: true, hasPDF: true, hasQuiz: true },
        { id: "noktalama", title: "Noktalama İşaretleri", hasVideo: true, hasPDF: true, hasQuiz: true },
        { id: "dil-bilgisi", title: "Dil Bilgisi (Dil Bilgisi ve Yapı Bilgisi)", hasVideo: true, hasPDF: true, hasQuiz: true },
        { id: "anlatim-bozukluklari", title: "Anlatım Bozuklukları", hasVideo: true, hasPDF: true, hasQuiz: true },
        { id: "anlatim-turleri", title: "Anlatım Türleri ve Düşünceyi Geliştirme Yolları", hasVideo: true, hasPDF: true, hasQuiz: true },
        { id: "metin-turleri", title: "Metin Türleri (Genel Kültür / Okuma Anlama)", hasVideo: true, hasPDF: true, hasQuiz: true }
      ]
    }
  ];
  ...
  const getOptimizedSearchQuery = (topicId: string): string => {
    const searchQueries: Record<string, string> = {
      "sozcukte-anlam": "TYT türkçe sözcükte anlam",
      "cumlede-anlam": "TYT türkçe cümlede anlam",
      "paragrafta-anlam": "TYT türkçe paragrafta anlam ana düşünce",
      "ses-bilgisi": "TYT türkçe ses bilgisi ses olayları",
      "yazim-kurallari": "TYT türkçe yazım kuralları imla",
      "noktalama": "TYT türkçe noktalama işaretleri",
      "dil-bilgisi": "TYT türkçe dil bilgisi yapı bilgisi",
      "anlatim-bozukluklari": "TYT türkçe anlatım bozuklukları",
      "anlatim-turleri": "TYT türkçe anlatım türleri düşünceyi geliştirme",
      "metin-turleri": "TYT türkçe metin türleri okuma anlama"
    };

    return searchQueries[topicId] || `TYT türkçe ${topicId}`;
  };
  ...
  const selectTopic = async (topicId: string) => {
    ...
    // Tour için data attribute ekle
    const topicButton = document.querySelector(`[data-topic-id="${topicId}"]`);
    if (topicButton) {
      topicButton.setAttribute('data-selected-topic', 'true');
    }
    ...
    if (currentSubTopic) {
      ...
      try {
        ...
        const cachedVideos = await getCachedVideos('tyt-turkce', topicId);
  ...
  const markTopicComplete = (topicId: string) => {
    ...
    if (!isAlreadyCompleted) {
      const topicButton = document.querySelector(`[data-topic-id="${topicId}"]`);
      if (topicButton) {
        topicButton.setAttribute('data-topic-completed', 'true');
      }
    }
  ...
  const playVideo = (video: YouTubeVideo) => {
    ...
    setTimeout(() => {
      ...
      if (player) {
        player.setAttribute('data-video-player', 'open');
      }
    }, 100);
  };
  ...
  return (
    <div className="min-h-screen text-white" style={{backgroundColor: '#0a0e27'}}>
                  ...
                  <div className="space-y-3 lg:space-y-4">
                    ...
                    >
                      ...
                      <button 
                        ...
                        onClick={() => {
                          // Test butonuna tıklandığında data attribute ekle
      ...
      <div data-video-player={isPlayerOpen ? "open" : "closed"}>
        <YouTubePlayer
          video={selectedVideo}
          isOpen={isPlayerOpen}
          onClose={closePlayer}
